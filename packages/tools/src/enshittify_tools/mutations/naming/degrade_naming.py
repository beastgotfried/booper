"""LangChain tool for degrading Python identifier names."""
...
def _create_degraded_name(index: int, blocked_names: set[str]) -> str:
    current = index

    while True:
        base = _BAD_NAMES[current % len(_BAD_NAMES)]
        suffix = current // len(_BAD_NAMES)
...
class _NameDegrader(ast.NodeTransformer):
...
def mutate_source(code: str) -> MutationResult:
    """Replace clear Python variable and argument names with vague names."""
    try:
        tree = ast.parse(code)
        ...
        return MutationResult(
            code=code,
            changed=False,
            summary="Code was not edited because it contains syntax errors.",
            edits=[],
    ...
    for original_name in candidate_collector.candidates:
        replacement = _create_degraded_name(len(rename_map), used_names)
    ...
    if not rename_map:
        return MutationResult(
            code=code,
            changed=False,
            summary="No safe naming degradation candidates were found.",
            edits=[],
            warnings=[],
        )

    renamed_tree = _NameDegrader(rename_map).visit(tree)
    ...
    return MutationResult(
        ...
        changed=True,
        summary=f"Degraded {len(rename_map)} identifier name(s).",
        edits=edits,
        warnings=[],
    )


def degrade_naming_source(code: str) -> MutationResult:
    """Compatibility helper for graph nodes that need structured output."""
    return mutate_source(code)


@tool
def degrade_naming(code: str) -> str:
    """Replace clear Python variable and argument names with vague names."""
